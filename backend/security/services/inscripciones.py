...
## falta ver el tema si estan activas estas inscripciones.
    ...
    def get_inscripciones_usuario(self, fecha, usuario_id):
        
        result = self.db.query(inscripcionesModel) \
                        .options(load_only(inscripcionesModel.id, inscripcionesModel.evento_id,
                                        inscripcionesModel.usuario_id, inscripcionesModel.fecha_inscripcion)) \
                        .filter(and_(EventosModel.fecha_inicio > fecha, inscripcionesModel.usuario_id == usuario_id)) \
                        .all()
        return [Inscripciones(**r.__dict__) for r in result]
